...
static int enumlookup(struct symlist *sl, unsigned char type, std::function<bool(const char *name)> name_callback) {
	Elf32_Sym *p;
	int i;

	for (i = 0, p = sl->sym; i < sl->num; i++, p++) {
		//LOGD("name: %s %x\n", sl->str+p->st_name, p->st_value)
		if (!name_callback(sl->str+p->st_name))
			return 0;
	}
	return -1;
}

static int enum_func_sym(symtab_t s, std::function<bool(const char *name)> name_callback) {
	unsigned char type = STT_FUNC;
	if (s->dyn && !enumlookup(s->dyn, type, name_callback))
		return 0;
	if (s->st && !enumlookup(s->st, type, name_callback))
		return 0;
	return -1;
}
...
extern int enum_func(pid_t pid, const  char *libn, std::function<bool(const char *name)> name_callback)
{
	...
	if (0 > enum_func_sym(s, name_callback)) {
		return -1;
	}
