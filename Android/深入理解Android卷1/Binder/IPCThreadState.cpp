...
restart:
        ...
        /**
         * TLS是Thread Local Storage的简称。
         * 这种空间每个线程都有，并且线程间不共享这些空间。
         * 通过pthread_getspecific/pthread_setspecific函数可以获取/设置这些空间中的内容。
         * 从TLS中获得保存在其中的IPCThreadState对象。
         * 有调用通过pthread_getspecific函数的地方，肯定也有调用通过pthread_setspecific的地方。
         */
        IPCThreadState* st = (IPCThreadState*)pthread_getspecific(k);
        if (st) return st;
        //new 一个IPCThreadState对象，构造函数中会调用pthread_setspecific。
...
void IPCThreadState::joinThreadPool(bool isMain)
{
    ...
    //注意isMain为true,我们则需要循环处理，把请求信息写道mOut中，待会儿一起发出去。
                                  ...
                                  Parcel* reply, uint32_t flags)
{
    ...
    if (err == NO_ERROR) {
        ...
        /**
         * 注意这里的第一个参数BC_TRANSACTION，它是应用程序想binder设备发送消息的消息码，
         * 而binder设备向应用程序回复消息码以BR开头。消息码的定义在binder_module.h中，
         * 请求消息码和回应消息码的对应关系，需要查看Binder驱动的实现才能将其理解清楚，我们这里暂时用不上。
         */ 
      ...
      mLastTransactionBinderFlags(0)
{
    ...
    clearCaller();
    //mIn和mOut是两个Parcel。把它看成事发送和接收命令的缓冲区即可。
    //每个线程都有一个IPCThreadState，每个IPCThreadState都有一个mIn和mOut，其中，
    //mIn是用来接收来自Binder设备的数据，而mOut则是用来存储发往Binder设备的数据的。
...
status_t IPCThreadState::talkWithDriver(bool doReceive)
{
    ...
    if (doReceive && needRead) {
        //接收数据缓冲区信息的填充，如果以后收到数据，就直接在mIn中了。
    ...
    int32_t handle, uint32_t code, const Parcel& data, status_t* statusBuffer)
{
    ...
    //把命令写到mOut中，而不是直接发出去，可见这个函数有带你名不副实。
    mOut.writeInt32(cmd);
...
status_t IPCThreadState::executeCommand(int32_t cmd)
{
    ...
    case BR_TRANSACTION:
        {
            ...
            if (tr.target.ptr) {
                //BnServiceManager从BBinder派生，这里的b实际上就是实现BnServiceManager的那个对象，这样就直接定位到业务层的对象。
                //关于它的作用，后面在讲。
    ...
    case BR_SPAWN_LOOPER:
        //特别注意，这里将受到来自驱动的指示以创建一个新线程，用于和Binder通信。
