...
{
    AutoMutex _l(mLock);
    //如果已经startThreadPool的话，什么都不操作。
    if (!mThreadPoolStarted) {
        mThreadPoolStarted = true;
        spawnPooledThread(true);//注意传进去的是true。
    }
}
...
sp<IBinder> ProcessState::getStrongProxyForHandle(int32_t handle)
{
    ...
    if (e != NULL) {
        ...
        if (b == NULL || !e->refs->attemptIncWeak(this)) {
            ...
            //对于新创建的资源项，它的binder为空，所以走这个分支。注意，hander的值为0。
            //hander为0代表的就是ServiceManager所对应的BBinder。
