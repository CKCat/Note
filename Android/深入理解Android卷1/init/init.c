...
void service_start(struct service *svc, const char *dynamic_args)
{
    ...
    /**
     * service一般运行于另一个进程中，这个进程也是init的子进程，所以启动service前需要判断
     * 对应的可执行未见是否存在，zygote对对应的可执行未见是/system/bin/app_process.
     */ 
...
int main(int argc, char **argv)
{
    ...
    open_devnull_stdio();
    /**
     * 设置init的日志输出设备为/dev/__kmsg__,不过该文件打开后会立即被unlink了，
    ...
    //通过读取/proc/cpuinfo得到机器的hardware名
    ...
    for(;;) {//init将进入一个无限循环
        ...
        if (!keychord_fd_init && get_keychord_fd() > 0) {//keychord设备初始化成功，则init也会关注来自这个设备的事件
