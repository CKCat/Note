...
static void parse_config(const char *fn, char *s)
{
    ...
    state.ptr = s;
    state.nexttoken = 0;
    state.parse_line = parse_line_no_op;//设置解析函数，不同的内容使用同的解析函数
    ...
    for (;;) {
        ...
        case T_NEWLINE:
            ...
            if (nargs) {
                ...
                if (kw_is(kw, SECTION)) {//判断关键字是不是SECTION
                    state.parse_line(&state, 0, 0);
...
static void *parse_service(struct parse_state *state, int nargs, char **args)
{
    ...
    //init维护了一个全局service链表，先判断是否已经有同名的service。
    svc = service_find_by_name(args[1]);
    if (svc) {//如果有同名的service，则不能继续后面的操作。
    ...
    //把zygote这个service加到全局链表的service_list中。
...
static void parse_line_service(struct parse_state *state, int nargs, char **args)
{
    ...
    case K_socket: {/* name type perm [ uid gid ] */
        struct socketinfo *si;//创建socket相关信息。
        if (nargs < 4) {
