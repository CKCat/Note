...
int AndroidRuntime::startVm(JavaVM** pJavaVM, JNIEnv** pEnv)
{
    ...
    ALOGD("CheckJNI is %s\n", checkJni ? "ON" : "OFF");
    if (checkJni) {//根据checkJni判断是否需要进行JNIcheck。
     ...
     * JNI calls.
     */
    //动用JNI_CreateJavaVM创建虚拟机，pEnv返回当前线程的JNIEnv变量。
...
void AndroidRuntime::start(const char* className, const char* options)
{
    ...
    //options的值是"start-system-server"
     ...
     */
    //将字符串"com.android.internal.os.ZygoteInit"中的"."替换成"/"。
    ...
    if (startClass == NULL) {
        ...
        //找到ZygoteInit类的mian函数的jmethodID。
        ...
        if (startMeth == NULL) {
            ...
            //3. 调用com.android.internal.os.ZygoteInit::main()函数
    ...
    free(slashClassName);
    //zygote退出，释放资源，在正常情况下，zygote不需要退出。
...
static int register_jni_procs(const RegJNIRec array[], size_t count, JNIEnv* env)
{
    for (size_t i = 0; i < count; i++) {
        if (array[i].mProc(env) < 0) {//调用数组的元素的mProc函数
#ifndef NDEBUG
