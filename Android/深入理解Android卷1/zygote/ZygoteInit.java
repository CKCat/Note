...
public class ZygoteInit {
    ...
    static void preload() {
        //预加载类，在/framework/base目录中的preloaded-classes文件中记录这预加载的类
        //此文件由framework/base/tools/preload工具生成，他判断每个类加载的时间是否大于1250微秒，超过这个时间就会被写到prelaoded-classes文件中。
        preloadClasses();
    ...
    public static void main(String argv[]) {
        try {
            ...
            runSelectLoop(); //4.zygote调用这个函数，等待请求

            closeServerSocket();//关闭socket
        } catch (MethodAndArgsCaller caller) {
            /**
            为什么使用抛异常的方法实现调用这个函数?这个调用是在ZygoteInit.main中，相当于Native的main函数，即入口函数，位于堆栈的顶层。如果不采用抛异常的方式，而是在invokeStaticMain那里调用，则会浪费之前函数调用所占用的一些调用堆栈。
             */
    ...
    private static void runSelectLoop() throws MethodAndArgsCaller {
        ...
        while (true) {
            ...
            try {
                ...
                //selectReadable内部调用select，使用多路服用I/O模型。
                //当有客户端连接成功或有数据时，则selectReadable就会返回。
            ...
            if (index < 0) {
                ...
                //有一个客户端连接上，客户端在zygote的代表是ZygoteConnection。
                ...
                //后续处理交个ZygoteConnection的runOnce函数完成。
