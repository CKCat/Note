...
- [HTTPS](#https)
- [HTTPS 的实现原理](#https-的实现原理)
  - [1. 证书验证阶段](#1-证书验证阶段)
  - [2. 数据传输阶段](#2-数据传输阶段)
- [为什么数据传输是用对称加密？](#为什么数据传输是用对称加密)
- [为什么需要 CA 认证机构颁发证书？](#为什么需要-ca-认证机构颁发证书)
- [浏览器是如何确保 CA 证书的合法性？](#浏览器是如何确保-ca-证书的合法性)
  - [1. 证书包含什么信息？](#1-证书包含什么信息)
  - [2. 证书的合法性依据是什么？](#2-证书的合法性依据是什么)
  - [3. 浏览器如何验证证书的合法性？](#3-浏览器如何验证证书的合法性)
  - [4. 只有认证机构可以生成证书吗？](#4-只有认证机构可以生成证书吗)
- [本地随机数被窃取怎么办？](#本地随机数被窃取怎么办)
- [用了 HTTPS 会被抓包吗？](#用了-https-会被抓包吗)
- [以下用简短的Q&A形式进行全文总结：](#以下用简短的qa形式进行全文总结)
